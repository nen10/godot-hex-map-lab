...
The sample is meant to mirror the Generate/Edit/Validate workflow output: construct or save a canonical document, validate it, then pass the `HexMapDocumentResource` to `examples/basic_runtime/runtime_query_example.tscn` or `runtime_query_sample.gd`. Saved paths are useful when the runtime layer needs to load the resource first.

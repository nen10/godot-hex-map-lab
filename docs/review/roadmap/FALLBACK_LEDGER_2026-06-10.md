...
| `LG-07` | mirror | Existing private `_generation_*` fields may remain only as transitional mirrors after `HexMapGenerationRunState`. | Generate run state / Generate Dock | `forbidden-normal-ui` | Mirrors are retired in `STATE-NEXT-11`; no run-state duplicated flags remain. | `docs/TEST.md` coverage for `STATE-10`, `STATE-NEXT-11`; `./tools/test.sh`; `python3 tools/verify_task.py --task STATE-NEXT-11 --head <branch>`. | `STATE-NEXT-11`. |

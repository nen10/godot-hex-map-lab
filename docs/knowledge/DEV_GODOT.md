...
- `Resource` には組み込みpropertyとして `resource_path` があるため、custom `Resource` scriptで `@export var resource_path` を再定義するとparse errorになる。依存関係は `resource: Resource` で保存し、表示用pathが必要な場合だけ参照先の組み込み `resource_path` を読む。

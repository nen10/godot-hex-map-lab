...
- `tests/test_editor_plugin.gd` QA-03 coverage: batch rowからのseed promotion、v2 document生成、generation snapshot metadata、保存/再読み込みroundtripを headless で検証する。
...
- `tests/test_debug_scenes.gd`: debug scene の生成形状切替、toric 表示 domain、loop path / cell hit / movement range 表示状態、runtime loop duplicate tile copy 状態、runtime helper による v2 `HexMapDocumentResource` path load と terrain / overlay / object / label 適用、`examples/basic_runtime/runtime_query_sample.gd` による document path load / weighted path / movement range query / object runtime export copy、`examples/basic_runtime/runtime_query_example.tscn` と `examples/editor_workflow/editor_workflow_example.tscn` のresource path load / runtime-safe preload / sample v2 document builder、対称生成 overlay が toric canvas 全体を網羅すること、phase2 grouping 表示用データを検証する。

...
- `tests/test_debug_scenes.gd`: debug scene の生成形状切替、toric 表示 domain、loop path / cell hit / movement range 表示状態、runtime loop duplicate tile copy 状態、runtime helper による v2 `HexMapDocumentResource` path load と terrain / overlay / object / label 適用、対称生成 overlay が toric canvas 全体を網羅すること、phase2 grouping 表示用データを検証する。

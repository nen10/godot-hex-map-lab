...
4. `Edit Mode` を `Shape` / `Wall / Floor` / `Floor Tile` / `Wall Tile` / `Object` / `Label` に切り替え、viewport上のhex cellをclickする。
5. Undo / Redoで document と TileMapLayer 表示が同時に戻ることを確認する。
6. `Save` した documentを再Loadし、shape、wall、tile override、object、labelが維持されることを確認する。
7. `Export` した `HexMapResource` をGenerate / runtime表示へ渡し、shapeとwallが維持されることを確認する。
8. `HexTileMapLayer` の toric loop表示では、複製表示されたcellが outline だけでなく floor / wall tile として表示されることを確認する。
9. 複製表示されたcellをclickしてcanonical cellが編集され、Undo / Redo 後も canonical cell と duplicate tile が同じ floor / wall 状態へ戻ることを確認する。

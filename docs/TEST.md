...
Editor Plugin 操作で複数機能の結合性を確認する任意検証は、アナログテストとして `docs/plan/ANALOG_TEST_POLICY_2026-06-01.md` に従い、操作手順マニュアルを `.test/analog_test/` 以下に作成する。アナログテスト文書は Test path の代替ではなく、ユーザー依頼時の追加検証記録として扱う。
...
8. `HexTileMapLayer` の toric loop表示では、複製表示されたcellをclickしてcanonical cellが編集されることを確認する。

...
- `tests/test_editor_plugin.gd` ASSET-32 coverage: `Create New...` の workspace button path が project resource を作成して asset context / row state を更新すること、explicit sample action の button path が sample source を選択して signal を出すこと、Settings sample row の未実装 `Open` / `Duplicate To Project` が visible UI から削除されていることを headless で検証する。

...
- `tests/test_hex_map_generation.gd`: rectangle/hexagon/toric square の生成、壁生成、割り込み可能な progress / cancel 付き壁生成、shape API の interrupt option、連結性回復、dense 連結性回復、terminal 接続、対称生成を検証する。dense 連結性回復は単純 bridge、toric shortcut、既存回復と同程度の削除数、flood より少ない削除数になる固定 fixture、対称 toric square の接続を検証する。Generation Radius 1 / 2 も larger radius と同じ対称生成フローを通り、radius 3 / 6 / 9 は `DrawAreaCenter -> DrawAreaFromCenter` 後の completion pass で canvas 内・split 分布・protected floor・連結性回復を満たすことを検証する。

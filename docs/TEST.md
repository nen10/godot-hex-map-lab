...
8. `Save` した documentを再Loadし、shape、wall、tile override、object、labelが維持され、`Save / Export` detail に path、resource class、cell count、wall count が表示されることを確認する。
9. `Export` した `HexMapResource` をGenerate / runtime表示へ渡し、shapeとwallが維持されることを確認する。
...
16. Godot Output Dockに `EditorUndoRedoManager.add_do_method` errorが出ないことを確認する。

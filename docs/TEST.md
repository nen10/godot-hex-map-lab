...
- `tests/test_hex_map_generation.gd`: rectangle/hexagon/toric square の生成、壁生成、連結性回復、terminal 接続、対称生成を検証する。Generation Radius 1 の対称生成は distribution 参照ではなく wall probability で完了することを検証する。
- `tests/test_hex_adapter.gd`: HexMapData から TileMapLayer 用 entry、表示用 local 座標、HexMapResource への変換を検証する。
- `tests/test_hex_tile_map_layer.gd`: `HexTileMapLayer` の `HexMapResource` 適用、セル照会、壁/床編集、local/hex 座標往復、経路・ハイライト・連結性 helper を検証する。
- `tests/test_editor_plugin.gd`: 生成ドックの対称 Hexagon / Generation Radius 1、Distribution Editor のプリセット値表示、`.tres` 読み込み、SpinBox 値に基づく色、window close の cancel flow を headless で検証する。

...
- `tests/test_editor_plugin.gd`: Hex Map Edit / Generate Dock の resource selection、FileDialog callback、shared editor session の target / document / selected resource / saved-path metadata、canonical HexMapDocumentResource の load / edit / save / export、button state、Dock scroll container、Copy Debug Report、catalog selector 由来の floor / wall / overlay / object state、typed object placement Undo/Redo、missing assignment validation、Validation dashboard の grouped issue rows / cell focus、debug report の validation summary、Target readiness、Target由来document保存、viewport input / UndoRedo / toric duplicate edit、modeごとの catalog / object / label state、Generation Dock の generation / validation / batch promotion / source registry / mapdata query / history save、HexCellButtonLayout / HexCellButtonPanel、Distribution Editor を headless で検証する。旧 path LineEdit や numeric fallback control の存在は test contract にしない。

...
つまり、実装済みコードを実際に操作した場合の期待動作を、ユースケース、操作手順、コードリーディング検証、ChatGPT agent 判定の順で確認するための文書作成時に従う指針である。
...
Editor Plugin 上の実操作で複数機能の結合性を確認できるユースケースを、擬似的な結合テスト・システムテストとして扱うマニュアル文書のこと
  ...
  - コードリーディング検証結果を受けたあと、ユーザーの判断によりChatGPT agent による操作テストを行う。
...
ChatGPT agent の判定は、計画文書を `docs/complete_on_test/` へ移動する自動根拠にはしない。完了扱いは、`docs/TEST.md` に記録された test、またはユーザーレビューで承認された範囲に基づいて判断する。

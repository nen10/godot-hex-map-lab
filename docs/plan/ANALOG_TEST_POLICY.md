...
このPolicy文書は、次に示すアナログテスト作成時の指針である、
つまり、対象コードを実際に操作した場合の期待動作を、ユースケース、操作手順、コードリーディング検証、ユーザーまたはChatGPT agent 判定の順で確認するための文書作成時に従う指針である。

### アナログテスト

Editor Plugin 上の実操作で複数機能の結合性を確認できるユースケースを、擬似的な結合テスト・システムテストとして扱うマニュアル文書のこと

  - アナログテストは実装完了要件とは独立に扱い、要件の洗い出し・機能改善のための能動的な開発サイクルを実現するテストケースである
  - アナログテストは[ユースケースのplanning, 操作手順作成, コードリーディング検証, ChatGPT agent による操作実施] の順によって実行される
  - code readingによりユースケースを実現する機能に不足が確認された場合、機能追加・修正項目を計画できるようにdocumentationする。
  - コードリーディング検証結果を受けたあと、ユーザーの判断により操作テストを行う。
  - ChatGPT agentはまだ画面操作によるGodot作業を行えないとの自己申告があったため、ユーザーがマニュアルに従って操作を行いテスト結果を報告する。
  - アナログテストの実行はユーザー判断に責任を負うものであり、アナログテストとして実行不能な機能・マニュアルがプロジェクトに存在することはプロジェクトの瑕疵ではありません。開発中に実行不能な"Operation Steps"が存在することは正常な開発プロセスです。
...
- `.test/analog_test/` 以下の操作手順マニュアルは、製品 manual でも仕様書でもない。ユースケースに基づく検証手順と観察記録である。
...
```text
.test/analog_test/<USE_CASE_ID>_ANALOG_TEST_<YYYY-MM-DD>.md
docs/review/<USE_CASE_ID>_CODE_READING_<YYYY-MM-DD>.md
```

必要に応じて、ANALOG_TEST内に ChatGPT agent 判定用情報を追記する。分量が大きい場合次のように分ける。

```text
.test/analog_test/<USE_CASE_ID>_CHATGPT_JUDGEMENT_<YYYY-MM-DD>.md
```
...
- ユースケース実施に際して必要な外部入力が、別stepにおいて作成されること・ユーザーのコントロール操作Jとして可能な状態であることを、着実に辿る。
...
3. Codex が Editor Plugin による結合ユースケースを作成し、`.test/analog_test/` に操作手順マニュアルを書く。

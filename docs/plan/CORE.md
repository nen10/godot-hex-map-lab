...
実行時間が長くなる連結性回復用の関数(`HexMapGenerator.restore_connectivity_sparse()`, `HexMapGenerator.restore_connectivity_dence()`)においても progress / cancel 分岐を実行できるようにしたい。アルゴリズムの実行速度を損なわないように注意する。

...
Current recommended next task: `LD2-02`.

Reason:

- `LD2-01` is complete and provides Level Document v2 typed resource fields.
- `LD2-02` and `LD2-03` are now READY because their dependency `LD2-01` is complete.
- Select `LD2-02` first because it appears earlier in the queue and provides migration behavior needed by later roundtrip tasks.

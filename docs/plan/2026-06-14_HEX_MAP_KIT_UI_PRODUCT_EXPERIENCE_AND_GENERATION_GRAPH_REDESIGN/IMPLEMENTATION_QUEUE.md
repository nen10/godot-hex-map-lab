...
| `REPAIR-21_NODE_PARAM_SCHEMA_CENTRALIZATION` | `SUPERSEDED` | `REPAIR-19`, `REPAIR-20` | Superseded 2026-07-03: absorbed into `GQM-03_SCHEMA_FOR_CONSOLIDATED_NODES` (`docs/plan/2026-07-02_GRAPH_QUALITY_MANAGEMENT_UX_REDESIGN/IMPLEMENTATION_QUEUE.md`). The schema target changed from the legacy 11 node types to the consolidated 4 node types (Terrain Generation / Item Generation / Set Operation / Result) decided in `DEPENDENCY_UX_PROPOSALS.md` Round 3/4; the extended declarations (`visible_when` / `label_by_mode` / `derived_default` / `affects` / `asset_kind`) and the defaults-always-valid principle carry over unchanged. | — | See `GQM-03`. |

...
| `REPAIR-21_NODE_PARAM_SCHEMA_CENTRALIZATION` | `READY` | `REPAIR-19`, `REPAIR-20` | Move method-dependent param schema (keys / visibility / control kind / options / range / default) out of inspector hardcoded matches into a declarative registry in the generation layer; inspector renders from schema. Custom editors (item_pool / shift pad / markov window / adjacency window) stay as control kinds referenced by schema | `hex_generation_node_types.gd` (or new `hex_generation_param_schema.gd`), `hex_map_build_node_inspector.gd`, headless schema tests | S: schema is headless-testable (per node type per method) and inspector `_param_*` match functions are removed; existing inspector tests keep passing. E: execution-method dependency rules are owned by the generation layer, not UI code (root fix of the old gen_dock coupling debt; design principle 1). |
| `REPAIR-22_SEMANTIC_RUN_GATE` | `READY` | `REPAIR-21` | Graph-native equivalent of old `generation_block_reason`: per-node `validate_semantics(node)` (empty adjacency rules, limited pool without positive limits, empty item pool, toric_passage on non-square input as warning) surfaced as node badge + Generate button disable/tooltip | `hex_generation_node_types.gd`, `hex_map_build_graph_canvas.gd`, `hex_map_build_screen.gd`, tests | S: a graph containing a semantically-blocked node disables Generate with a visible reason and marks the offending node; resolving the issue re-enables Generate. E: user sees why generation is blocked before running (old Generate block-reason UX parity, upgraded to node granularity). |
| `REPAIR-23_ADJACENCY_SCAN_PARITY_REAUDIT` | `READY` | `REPAIR-15` | Re-audit adjacency scan parity after the perf-branch scan redesign (border-start / center-arc seeds with generated references) and verify markov custom distribution parity vs old `_current_distribution`; refresh audit matrix #15/#7 with tests fixing the intended behavior | `hex_map_generator.gd` audit + `tests/test_hex_map_generation.gd`, matrix update | S: scan-order bias status is re-established against the current implementation (not the stale 2026-06-24 findings) with a regression test. E: matrix #15/#7 are `migrated` or explicitly re-scoped, no stale findings remain as implementation guidance. |

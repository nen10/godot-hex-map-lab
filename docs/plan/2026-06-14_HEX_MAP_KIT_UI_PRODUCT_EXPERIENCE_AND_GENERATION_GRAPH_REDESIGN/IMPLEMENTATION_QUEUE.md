...
| `REPAIR-17_ADJACENCY_RULES_WINDOW` | `COMPLETE` | `REPAIR-15` | Replace probability_rules text field with structured Adjacency Rules window/presets | inspector/modal + rules resource/tests | S: hex-direction toggles, wall count/component count controls, preset dropdown, structured rules passed to generator. E: user configures adjacency rules visually without raw text. |
| `REPAIR-18_MARKOV_DISTRIBUTION_WINDOW` | `COMPLETE` | `REPAIR-15` | Restore Markov Mesh custom distribution editor for Wall node and wire custom_distribution state | inspector/modal + wall generator params/tests | S: markov_mesh shows edit-distribution action and passes custom_distribution to core. E: graph Wall Field can use preset or custom Markov distribution like old Generate intent. |
| `REPAIR-19_ITEM_METHOD_FIELD_WIRING` | `COMPLETE` | `REPAIR-15` | Item Generator pool rows follow placement_method (weighted=weight 0..1 / limited=core-visible `limit` int), rows re-render on method switch, adjacency exposes `item_name` and hides the pool editor | `hex_map_build_node_inspector.gd`, `tests/test_generation_graph.gd`, `tests/test_editor_generation.gd` | S: limited pool entries reach core `_normalize_limit_item_pool` and place exact counts via graph run; method switch relabels rows without losing the other method's values. E: user can configure limited counts and adjacency item names from Build inspector (old Generate item limit / item name parity). |
| `REPAIR-20_CONNECTIVITY_TORIC_OWNERSHIP` | `COMPLETE` | `REPAIR-15` | Move toric topology ownership from Shape to Connectivity (`toric_passage` param; wrap only on a full square cell set), remove Shape `toric` param and stale defaults | `hex_generation_node_types.gd`, `hex_map_build_node_inspector.gd`, `hex_generation_preset.gd`, `hex_map_build_graph_canvas.gd`, tests | S: toric_passage=true on square shape yields cyclic_size=size, false leaves flat, non-square refuses wrap; Shape inspector no longer offers toric. E: toric passage is managed where it is semantically used (user decision 2026-07-02), matching old symmetric-square `connect_toric` behavior. |
| `REPAIR-21_NODE_PARAM_SCHEMA_CENTRALIZATION` | `READY` | `REPAIR-19`, `REPAIR-20` | Move method-dependent param schema (keys / visibility / control kind / options / range / default) out of inspector hardcoded matches into a declarative registry in the generation layer; inspector renders from schema. Custom editors (item_pool / shift pad / markov window / adjacency window) stay as control kinds referenced by schema. **Scope extended (2026-07-03, dialogue R2-3/R3): schema also declares the dependency-transition model — `visible_when` / `label_by_mode` (e.g. wall_probability = "Initial Probability" under markov) / `derived_default` / `affects` (linked-update list, headless-testable) / `asset_kind` — and guarantees the "defaults always valid" principle (every param default-filled; default-only Generate always succeeds; empty rules/pool are unconstructible). No gate UI.** | `hex_generation_node_types.gd` (or new `hex_generation_param_schema.gd`), `hex_map_build_node_inspector.gd`, headless schema tests | S: schema is headless-testable (per node type per method, including `affects` linkage) and inspector `_param_*` match functions are removed; existing inspector tests keep passing; default-only graphs generate successfully for every node type/method. E: execution-method dependency rules and their linked transitions are owned by the generation layer, not UI code — the old gen_dock state-evaluator cascade is inherited declaratively without requiring user 暗黙知 (design principle 1; `docs/plan/2026-07-02_GRAPH_QUALITY_MANAGEMENT_UX_REDESIGN/RESOURCE_MODEL.md` §5). |
| `REPAIR-22_SEMANTIC_RUN_GATE` | `SUPERSEDED` | `REPAIR-21` | ~~Graph-native equivalent of old `generation_block_reason`~~ Superseded 2026-07-03 (dialogue R2-3): gates/block-reason badges are not built. Replaced by the "defaults always valid" principle absorbed into `REPAIR-21` — invalid states are made unconstructible instead of detected. | — | See `REPAIR-21` extended acceptance. |
| `REPAIR-23_ADJACENCY_SCAN_PARITY_REAUDIT` | `BACKLOG` | `REPAIR-15` | Re-audit adjacency scan parity after the perf-branch scan redesign (border-start / center-arc seeds with generated references) and verify markov custom distribution parity vs old `_current_distribution`; refresh audit matrix #15/#7 with tests fixing the intended behavior. **Demoted to tail priority (2026-07-03, dialogue R2-5): UX 上の意義が不明確. Do not select while structural UX work (RESOURCE_MODEL slices) is open.** | `hex_map_generator.gd` audit + `tests/test_hex_map_generation.gd`, matrix update | S: scan-order bias status is re-established against the current implementation (not the stale 2026-06-24 findings) with a regression test. E: matrix #15/#7 are `migrated` or explicitly re-scoped, no stale findings remain as implementation guidance. |

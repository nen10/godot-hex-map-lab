...
- README.mdの内容整備
- 現状のマニュアルについて
  - EDITOR PLUGIN用マニュアルは実装内容とのズレが大きい。
  - SCRIPTING用マニュアルについて、重要な内容がわかりにくい。説明順はこんな感じの方が構造が見やすい。
  ```
    - 座標データクラス
    - 座標/ベクトルの集まり(List, Set) 取得方法
      - disc, ring, 近傍(toric/non-toric) 等のList
        - 座標と座標は加算できないが、少なくとも一方がベクトルであれば加算つまり、平行移動できることに注意する。
          - Listは演算を想定しているが、Setは演算を想定していないかもしれない。よくわからないのでマニュアルで確認したい。
      - 壁集合: 壁の生成方式/マップ形状
        - マップのデータ構造
        - 壁集合に対する操作 : マップとしての連結性回復
      - 経路: 経路アルゴリズム
  ```
    - ファイルごとの各関数に対する説明はAPIリファレンスで回収し、概念説明を中心に置く。
- アドオン公開用リポジトリ内に配置するための、不特定ユーザー向けドキュメントの整備

## TEST

- "unity_reference_groups は同一トーラス参照を持つエントリをグループ化し、どの座標がトーラス空間で「同一」か可視化します。phase2_outer_mod_groups は phase 2 の外側境界補正におけるペア/トリプル関係を示します。"
  - この理解は誤りではないかと思い始めています。
  - なんらかの意味で同じもの(たとえば"トリプルを持っている"同士、"ペアを持っている"同士)かもしれませんが、トーラス上で同じである点ではない。それは十分遠く、cyclic_sizeに従った距離に配置されるはずです。
  - バグ修正の過程で生じた方便が今は、生成タイミングとしてのペア・トリプルという意味でしかない領域に対する混乱したテストに変化している気がします。
